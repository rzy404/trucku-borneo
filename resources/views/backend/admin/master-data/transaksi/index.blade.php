@extends('layouts.app')
@section('title', session()->has('bahasa') ? GoogleTranslate::trans("Master Data - Transaksi | TrucKu Borneo",
session()->get('bahasa')) : GoogleTranslate::trans("Master Data - Transaksi | TrucKu Borneo", 'id'))
@section('css')
...
<div class="container-fluid">
    <div class="page-titles">
        <h4>{{ session()->has('bahasa') ? GoogleTranslate::trans("Transaksi", session()->get('bahasa')) :
            GoogleTranslate::trans("Transaksi", 'id') }}</h4>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="javascript:void(0)">{{ session()->has('bahasa') ?
                    ...
                    'id') }}</a></li>
            <li class="breadcrumb-item active"><a href="{{ route('transaksi.index') }}">{{ session()->has('bahasa')
                    ?
                    GoogleTranslate::trans("Transaksi",
                    session()->get('bahasa')) : GoogleTranslate::trans("Transaksi",
                    'id') }}</a></li>
        </ol>
    </div>
    <div class="col-12">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th>{{ session()->has('bahasa') ?
                                    GoogleTranslate::trans("ID Transaksi",
                                    session()->get('bahasa')) : GoogleTranslate::trans("ID Transaksi",
                                    'id') }}</th>
                                <th>{{ session()->has('bahasa') ?
                                ...
                                <th>{{ session()->has('bahasa') ?
                                    ...
                                    'id') }}</th>
                                <th>{{ session()->has('bahasa') ?
                                    GoogleTranslate::trans("Status",
                                    session()->get('bahasa')) : GoogleTranslate::trans("Status",
                                    'id') }}</th>
                                <th>{{ session()->has('bahasa') ?
                                    GoogleTranslate::trans("Pembayaran",
                                    session()->get('bahasa')) : GoogleTranslate::trans("Pembayaran",
                                    'id') }}</th>
                                <th>{{ session()->has('bahasa') ?
...
<!-- Modal -->
@include('backend.admin.master-data.transaksi.modal')
@endsection
@section('script')
<script>
    ...
    $(document).on('click', "#detailTransaksi", function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#detailTransaksi .jarak_tempuh').html("Jarak Tempuh :");
                $('#detailTransaksi .data_jarak').html(data.jarak_tempuh + " Km");
                $('#detailTransaksi .lama_sewa').html("Lama Penyewaan :");
                $('#detailTransaksi .data_lamaSewa').html(data.lama_sewa + " Hari");
