@extends('layouts.app')
@section('title', session()->has('bahasa') ? GoogleTranslate::trans("Master Data - Metode Pembayaran | TrucKu Borneo",
session()->get('bahasa')) : GoogleTranslate::trans("Master Data - Metode Pembayaran | TrucKu Borneo", 'id'))
@section('content')
<div class="container-fluid">
    <div class="page-titles">
        <h4>{{ session()->has('bahasa') ? GoogleTranslate::trans("Metode Pembayaran", session()->get('bahasa')) :
            GoogleTranslate::trans("Metode Pembayaran", 'id') }}</h4>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="javascript:void(0)">{{ session()->has('bahasa') ?
                    ...
                    'id') }}</a></li>
            <li class="breadcrumb-item active"><a href="{{ route('metodepb.index') }}">{{ session()->has('bahasa')
                    ?
                    GoogleTranslate::trans("Metode Pembayaran",
                    session()->get('bahasa')) : GoogleTranslate::trans("Metode Pembayaran",
                    'id') }}</a></li>
        </ol>
    </div>
    <!-- Datatable -->
        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ session()->has('bahasa') ?
                    GoogleTranslate::trans("Data Metode Pembayaran",
                    session()->get('bahasa')) : GoogleTranslate::trans("Data Metode Pembayaran",
                    'id') }}</h4>
                <button type="button" class="btn btn-rounded btn-success" data-toggle="modal"
                    data-target="#ModalMetodePembayaran" id="AddMetode_Pembayaran">
                    ...
                    <table id="data-MetodeBayar" class="display min-w850">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex">
                                        <a class="btn btn-primary shadow btn-xs sharp mr-1" id="editMetode"
                                            data-id="{{ $mp->id }}" data-toggle="modal"
                                            data-target="#ModalMetodePembayaran"><i class="fa fa-pencil"></i></a>
                                        <form method="POST" action="{{ route('metodepb.delete', $mp->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <input name="_method" type="hidden" value="DELETE">
                                            <button type="submit" class="btn btn-danger shadow btn-xs sharp"
                                                id="deleteMetode"><i class="fa fa-trash"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
...
<!-- Modal -->
@include('backend.admin.master-data.metode-pembayaran.modal')
@endsection
@section('script')
<script>
